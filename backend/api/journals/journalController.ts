...
export const createJournalChat: RequestHandler = async (
  ...
  try {
    // Validate the incoming request data using journalChatSchema
    const { entry_text, mode } = journalChatSchema.parse(req.body);
    ...
    // Call the Gemini API to generate a response with the provided mode and entry text
    const response = await fetch(
      `https://generativelanguage.googleapis.com/v1beta/models/gemini-2.0-flash:generateContent?key=${apiKey}`,
      {
        ...
        body: JSON.stringify({
          contents: [{ parts: [{ text: `Mode: ${mode}. User: ${entry_text}` }] }],
        }),
      }
    );

    if (!response.ok) {
      res.status(response.status).json({ error: "Error from Gemini API" });
      return;
    }
    ...
    // Optionally: Save the journal chat (entry_text and aiReply) to your database here
